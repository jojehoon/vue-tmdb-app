  ...
  <transition name="modal">
    <section class="modal" v-modal @click.stop="SET_MODAL" @scroll.prevent>
      <div class="modal__inner" @click.stop> 
        ...
        <button class="modal__button--close" type="button" @click="SET_MODAL"><span class="a11y">close</span></button>
      </div>
    </section>
  </transition>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['GET_MODAL', 'GET_MOVIE']),
    ...
    watch: {
      GET_MODAL: function(newVal) {
        // FIXME window, mac os별 크롬 브라우저에서 scroll의 너비 대응 필요
        if(newVal){
          // setTimeout(() => {
